...
class SpatialReasoner {
  ...
  // dynamic parameters
  private var vessel_coords: Map[String, SortedMap[Long,Seq[String]]] = Map()
  private var vessel_vel: Map[String, SortedMap[Long,Seq[String]]] = Map()
  private var vessel_gaps: Map[String, SortedMap[Long,Seq[String]]] = Map()
  ...
  private var vessels_areas: Map[(String,String),Seq[(String,Long)]] = Map()
  private var proximity_vessels: Map[(String,String),Seq[(Long,Long)]] = Map()
  ...
  def run(input: (Seq[Reader.Main.InputHappensAt], Seq[Reader.Main.InputHoldsAt], Seq[Reader.Main.InputHoldsFor]),
          outputFile: String,
          start: Long, end: Long, lastTime: Long): Long = {
  ...
  private def processInput(): Unit = {
    ...
    _input foreach { ie =>
      if (getEventType(ie)) {
        ...
        val time: Long = ie._3
        if (te.head == "coord") {
          val m: SortedMap[Long, Seq[String]] = vessel_coords.getOrElse(id,SortedMap())
          vessel_coords += (id -> (m + (time -> te)))
          }
        else if (te.head == "velocity") {
          //println(ie)
          val m: SortedMap[Long, Seq[String]] = vessel_vel.getOrElse(id,SortedMap())
          vessel_vel += (id -> (m + (time -> te)))
        }
        else {
          val m: SortedMap[Long, Seq[String]] = vessel_gaps.getOrElse(id,SortedMap())
  ...
  private def getEventType(event: (Data.InstantEventId, Seq[String], Long)): Boolean = {
  ...
  private def createProjections(): Unit = {
    ...
    vessel_coords foreach {
      x =>
        val lastTime: Long = x._2.lastKey
  ...
  private def createTrajectories(): Unit = {
    ...
    vessel_coords foreach { x => val m = x._2.toList
      var j : SortedMap[(Long,Long), Seq[String]] = SortedMap()
  ...
  def closestPointOfApproach(v1_c:List[Coordinate], v2_c:List[Coordinate], time: (Long,Long)): List[(Long,Long)] = {
    ...
    var ftcpa: ListBuffer[Long] = ListBuffer()
    ...
    if (cpa == 0.1 && tcpa > 0 && tcpa < Dt) {
      ftcpa += (time._1 + math.round(tcpa),time._1 + (math.round(tcpa)+1))
      ...
      if (!(roots.head.isNaN && roots.last.isNaN)) {
        ...
        if (roots.head < 0) {
          ...
          ftcpa += time._1 + math.floor(roots.head).toLong
        }

        if (roots.last >= 0 && roots.last <= Dt) {

          ftcpa += time._1 + math.ceil(roots.last).toLong
  ...
  def transformSegments(endpoints: List[(Data.Intervals,Seq[Double])], time: (Long,Long)): Geometry = {
  ...
  def estimateTimeOfInter(coord: Array[Coordinate], endpoints: Array[Coordinate], time: Data.Intervals, poly: Polygon):
    List[(String,Long)] =
  {
    var lip: ListBuffer[(String,Long)] = ListBuffer()
    ...
    val ftip = (math.round(time.head + ts), math.round(time.head + te))
  ...
  /**
    ...
    * @return
    */
  def getProjections: Map[String,Map[Long,(Seq[String], Seq[String])]] = projections

  def getProximity: Map[(String,String),Seq[(Long,Long)]] = proximity_vessels

  /**
    * Updates trajectories
    */
  def updateTrajectories(traj: Map[String,Map[Long,(Seq[String], Seq[String])]]): Unit = {
  ...
  def updateProximity(prox: Map[(String,String),Seq[(Long,Long)]]): Unit = {
